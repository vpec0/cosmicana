...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	  ...
	  if (piercer)
	      hists_pierce[(ix*NVOX_Y+iy)*NVOX_Z + iz] -> Fill(dedx[ip]);
      }
   }
   ...
   auto outf = TFile::Open(Form("%shists_dedx_by_voxel.root", gOutpref.Data()),"recreate");
...
int process(const char* fname, const char* outpref = "")
{
    gOutpref = outpref;
    gStyle -> SetOptStat(0);
