 ...
 *
 * Selecting reco tracks that travel from APA to CPA
 *
...
void process_dqdx_vs_x_APACPA_crossers(const char* fname = "",
				       const char* outpref = "",
				       int batchNo = 21, int Nruns = 10)
{
    enum {
	Dqdx_vs_x = 0,
	Dqdx_vs_x_plane0,
...
#define H2(name, title, nbinsx, lowx, highx, nbinsy, lowy, highy) hists[name] = new TH2F(#name, title, nbinsx, lowx, highx, nbinsy, lowy, highy)
	...
	for (int itrack = 0; itrack < ntracks; ++itrack) {
	    ...
	    // check if track crossed APA and CPA
	    int napa, ncpa;
	    int apa[20], cpa[20];
	    TVector3 start(evt->trkstartx_pandoraTrack[itrack],
			   evt->trkstarty_pandoraTrack[itrack],
			   evt->trkstartz_pandoraTrack[itrack] );
	    TVector3 end(evt->trkendx_pandoraTrack[itrack],
			 evt->trkendy_pandoraTrack[itrack],
			 evt->trkendz_pandoraTrack[itrack] );
	    crossingAPACPA(start, end, napa, apa, ncpa, cpa);

	    if ( ! napa || ! ncpa ) // not crossing either APA or CPA
		continue;
    ...
    //***** Output file *****
    auto outf = TFile::Open(Form("%sAPACPA_crossers_anahists.root", outpref), "UPDATE");
